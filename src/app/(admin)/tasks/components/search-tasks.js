...
export default function SearchTasks({ onTaskSearching }) {
	...
	const handleSearch = async () => {
		...
		onTaskSearching(keyword);

		setKeyword(""); // Reset keyword
	};

	return (
		<div className="flex flex-col @md:flex-row absolute inset-y-0 right-0 pe-35 w-auto">
			<button
				type="button"
									...
									<div className="sm:flex sm:items-start">
										...
										<div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
											...
											<p className="mt-2 text-sm text-gray-500">
												Stay on top of your workflow with our Task Tracker App!
												Simply enter a keyword to search for tasks by title or
												description.
											</p>
										</div>
									</div>
										...
										<Fieldset className="space-y-6 rounded-xl px-6 pb-6 sm:px-10 sm:pt-4">
											<Field>
												<Input
													...
													className={clsx(
														"mt-3 block w-full rounded-lg border-3 border-gray-100 bg-white/5 py-1.5 px-3 text-sm/6 text-gray-500",
